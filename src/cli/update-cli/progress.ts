...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g @m37labs/mightyclaw@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g @m37labs/mightyclaw@latest --omit=optional");
  }
