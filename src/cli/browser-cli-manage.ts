...
async function runBrowserToggle(
  ...
  defaultRuntime.log(info(`MC browser [${name}] running: ${status.running}`));
}
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        if (!result.moved) {
          defaultRuntime.log(info("MC browser profile already missing."));
          return;
        }
        const dest = result.to ?? result.from;
        defaultRuntime.log(info(`MC browser profile moved to Trash (${dest})`));
      });
    });
    ...
    .action(
      async (
        ...
        await runBrowserCommand(async () => {
          ...
          defaultRuntime.log(
            info(
              `MC created profile "${result.profile}"\n${loc}\n  color: ${result.color}${
    ...
    .action(async (opts: { name: string }, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        const msg = result.deleted
          ? `MC deleted profile "${result.profile}" (user data removed)`
          : `MC deleted profile "${result.profile}" (no user data found)`;
